...
#include "./loganov_a_qsort_std.h"
TEST(Q_SORT_STD_TESTS, Test1) {
...
TEST(Q_SORT_STD_TESTS, Test5) {
    std::vector<double> vec;
    std::vector<double> vec2;
    // time_t t1, t2, t3, t4;
    vec = getRandomVector(756);
    vec2 = copyvec(vec);
    // t1 = clock();
    Hoarsort(&vec, 0, static_cast<int>(vec.size())-1);
    // t2 = clock();
